...
const HomePage = () => {
  ...
  if (!cocktails) {
    return (
      <Layout>
        <h1 className="justify-content-center">No Cocktail Found</h1>
      </Layout>
    );
  }
  return (
    <>
      <div className="container ">
        <div className=" d-flex row fw-lighter justify-content-center">
          {modifiedCocktails.map((item) => (
            <div className="col-md-3 mt-3 m-1" key={item.id}>
              <div className="card" style={{ width: "18rem" }}>
                <img src={item.img} className="card-img-top" alt={item.name} />
                <div className="card-body">
                  <h5 className="card-title text-center btn-dark">{item.name}</h5>
                  <h4 className="card-title text-center ">{item.glass}</h4>
                  <p className="card-text text-center ">{item.info}</p>
                  <Link to={`/products/${item.id}`} className="btn btn-primary bg-danger ">
                    DETAILS
                  </Link>
                </div>
              </div>
